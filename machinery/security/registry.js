...
const sinks = [
  ...
  },
  {
    ...
    cwe: 'CWE-89',
    owasp: 'A03:2021',
    note: 'Prisma\'s explicitly-unsafe raw APIs. Deliberately NOT matching $queryRaw/$executeRaw: those are tagged templates that parameterize their interpolations, so flagging them is a false positive. See registry.test.js.',
  },
  {
    id: 'sql.sqlite.exec',
    root: { method: /^exec$/ },
    ...
    cwe: 'CWE-89',
    owasp: 'A03:2021',
    note: 'better-sqlite3 / sqlite3 db.exec(sql) executes a statement batch and cannot be parameterized at all — the only remedy is not to build it from input.',
  },
  {
    id: 'sql.prepare',
    root: { method: /^prepare$/ },
